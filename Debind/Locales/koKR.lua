...
L["OVERVIEW"] = "개요"
-- 근거는 enUS 쪽 주석에.
L["OVERVIEW_DESC"] = "지금 키가 지정된 모든 행동을 키별로 묶어서 보여줍니다. 한 키 안에서는 Debind가 훑는 순서대로 늘어놓습니다.|n|n여기 있는 것은 지금 이 캐릭터와 전문화에서 그 키를 누르면 실제로 일어날 일입니다. 오른쪽에서 다른 탭이나 전문화를 열어도 이 목록은 바뀌지 않고, 키가 없는 행동은 맨 끝에 따로 모여 나옵니다 - 문자열로 받아온 것은 같이 온 묶음 그대로, 나머지는 한 덩어리로. 아직 받아들이지 않은 것은 목록에는 있어도 어떤 키에도 안 걸립니다."
