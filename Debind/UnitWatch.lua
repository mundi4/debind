...
do
    ...
    ---
    --- `Misc.lua`'s `CannotStand` reads it to mark a binding `dead` where "only while solo" is set
    ...
    --- raid-only would mark a binding that fires `dead`, and `BuildKeyMap` leaves it off the key --
