...
> 상태 (2026-08-19): **조사만 했다. 코드는 한 줄도 안 건드렸고 아래는 전부 제안이다.**
>
...
**A. `binding`은 재사용되는데 wipe를 안 한다.** 캐시에서 꺼내 제자리에서 덮어쓰는데, 조건부로만
...
**B. 무효화 프로토콜이 도착지에서 꺼져 있다.** `action._dirty`를 **열한 곳이 쓰고 영 곳이 읽는다**
...
`v3.1.6`이 최신 태그이고, `DB_VERSION = 5`를 넣은 커밋(`Give each unit condition axis its own
field`)은 **어느 태그에도 안 들어 있다.** dbver 5는 한 번도 안 나갔다. 밖에 있는 사람은 전부
4 이하이므로 **이름 바꾸기와 `conditions` 중첩을 기존 `dbver <= 4` 단계 안에 넣으면 된다.**

`.zzz/savedvars/dbver4` 표본도 4라서 새 단계를 그대로 태운다.

대가는 하나. **이미 dbver 5로 올라간 설치본은 게이트에 막힌다**(`Profile.lua`의
`dbver >= DB_VERSION`이면 즉시 리턴). 지금 코드를 한 번이라도 띄운 라이브/PTR 계정의
SavedVariables는 지우거나 손으로 `dbver`를 4로 내려야 한다.

`tests/migration_spec.lua`의 `"dbver 5 ..."` 테스트 이름들은 `<= 4` 단계를 가리키게 고쳐야 한다.
이름만.

**그런데 이 창은 서두를 이유가 못 된다.** 3.2가 나가면 5가 밖으로 나가고 `Export.lua`의
`SCHEMA_VERSION` 1 창도 같이 닫히는데, **3.3이 둘 다 다시 연다.** 커스텀 상태 재설계가
...
- **`_dirty`를 지울지 되살릴지.** 되살리면 `GetBindingInfoForAction`이 진짜 캐시가 되는데,
  그러려면 열한 곳이 정확한지 먼저 확인해야 한다. 지금 실행된 적 없는 코드다.
- **`binding.reactions`를 없앨지.** `checkedUnits.hover`에서 그때그때 읽으면 세 겹이 두 겹이
  되는데, 솔버 스펙이 손으로 만든 바인딩에 이 필드를 넣고 있다. 스펙 쪽을 먼저 봐야 한다.
