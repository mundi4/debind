...
**페이로드를 그대로 저장하자는 것이 안건이다.** 이번 대화에서 문자열을 저장할 이유를 찾다가 넷을
댔고 넷 다 죽었다. 다시 세우지 않도록 근거를 적어둔다.
...
**남은 차이는 디스크 크기 하나다.** 문자열은 압축돼 있고(주석에 300액션이 약 1.4KB) 페이로드는
펼쳐진 Lua 테이블로 앉는다. 이 애드온이 갈라져 있는 이유가 SavedVariables를 통째로 읽는 비용이라
무관하지 않다. **그 크기는 안 재봤다. 재는 것이 이 안건의 다음 걸음이다.**
