...
- **활성 레이어의 액션이 걸린 키는 빌드에서 빠진 액션뿐이어도 잡는다** (2026-09-15, 소유자). 전문화 조건,
  ...
  전에 `KeysToHold`에 적고, `UpdateBindingsMap`이 그 키를 BLOCK만으로 건다. 놓는 것은 셋이다. hover가 걸린 마우스
  버튼 액션(개체창으로 나가고 키를 안 잡는다), 양보 중인 키, `key` 갈래 이슈(맨 왼쪽/오른쪽 클릭과 게임 메뉴 키는
  잡으면 월드 클릭이나 ESC가 죽는다). 받아들이기 전의 가져온 액션은 원래대로 키에 안 닿는다. 잡혔는지는
  `IsKeyOurs`가 답한다.
...
(2026-09-13, 소유자), 그런 설정이 애초에 안 된다. hover 없이 두 버튼에 걸면 ERROR 이슈
(`BINDING_ISSUE_NOT_SUPPORTED_MOUSE_BUTTON`, `Misc.lua`의 `IsKeyInvalidForAction`)라 키에 안 오르고, hover가 걸린
마우스 버튼 레코드는 `holdsKey`가 거짓이라 클릭캐스팅으로만 간다(`UpdateBindings.lua`의 `PrepareKeyBindings`). 월드
클릭과 카메라 조작이 막히는 일은 없다.
