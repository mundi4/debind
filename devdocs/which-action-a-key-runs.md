...
쌍둥이는 없다. 그래서 그 키에서는 hover 쌍둥이가 없으면 넷이 다 없는 것이고 WARNING이다.
...
| 47 | 수식키 없는 왼클릭, `hoverCast.aim = "skip"` | 개체창 클릭 | 키를 안 잡는다. 개체창 자체의 클릭으로 간다. WARNING. 넷 다 끈 것과 다른 문구로, 이 키에서는 Hover Cast만 선다고 말한다 |
...
않는다(`Constants.BINDING_ISSUE_GRADES`). 키에는 그 액션이 없는 것과 같다. **그래도 키는 잡는다**
(2026-09-17, 소유자). 활성 레이어의 액션에 키가 있으면 그 키는 우리 것이다. 놓는 것은 키에 대한 이슈(게임
메뉴 키)와, 늘 개체창 클릭으로만 서는 수식키 없는 왼클릭·오른클릭(§7)이다(S5 #42, #43). 이슈 검사의 갈래 이름은
