...
**넷을 다 끈 액션은 어느 층에도 안 굽는다.** 막지 않는다(2026-09-16, 소유자). 바인딩을 하나도 안 만들고,
**이슈가 아니다**(2026-09-17, 소유자). 사용자가 일부러 끈 것일 수 있는데, 표시를 지우려면 켜고 싶지 않은
누름을 켜야 하기 때문이다. 순서 목록 이유 칸에 "실행되지 않음"이 붙고, 툴팁의 Cast Options 줄 아래에 그
이유가 흐린 줄로 선다(`GetCastingOffReason`, `legacy/reorganizing-binding-issues.md` §3-3). 키에는 그 액션이 없는
것과 같다. **그래도 키는 잡는다** (2026-09-17, 소유자). 활성 레이어의 액션에 키가 있으면 그 키는 우리 것이다.
