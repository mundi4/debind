...
`BuildUnitStates`가 솔버 상자를 직접 좁히는 것이라 `IsConditionalBinding`이 볼 일이 없고, 그래서
순서(§2)의 조건 유무에도 안 든다.
...
각 줄이 헤드리스 테스트 하나다. 따로 적지 않은 것은 대상 안 고름, 조건 없음, 키보드 키, 설정 탭 모드
Unit Frames, 계정의 두 조합키 켬이다. **이 표의 "기본"은 Hover Cast를 Cast on pointed unit으로 켠
액션이다.** 표가 재는 것이 가리킨 누름이라 그렇고, 액션을 새로 만들었을 때의 기본값은 그 반대인 Off다(§6).
Off인 줄은 그렇게 적는다. "다음"은 그 액션에 바인딩이 없어 키의 다음 액션이 받는다는 뜻이고, 키에 그
액션뿐이면 아무 일도 안 한다.
...
| 8 | `mouseover` [없음], 설정 탭 Mouseover | 월드 유닛 가리킴 | 다음 |
| 9 | `unitframe` [없음] | Self Cast Key를 쥔 채 개체창 가리킴 | 다음. **조건은 쌍둥이 셋이 다 물려받는다.** 가리킨 누름만 빼고 쥔 누름은 남기는 값은 없다 |
...
| 14 | Hover Cast 끔 | 개체창 가리킴 | 원본, 게임이 놓음. 쌍둥이가 없어 4층에서 받는다. 같은 키에 켠 액션이 있으면 그 액션이 먼저다 |
...
| 41 | `selfCastKey`와 `focusCastKey` Off, Hover Cast 끔, `normalCast = false` | 어느 누름이든 | 다음. 이 액션은 어느 층에도 없음. Cast Options에 WARNING |
...
조건이 되어 조건 유무 물음에 든다(§0).

조건 유무는 원본 바인딩의 조건 표로 센다. 거기에 하나를 더한다(`MakeOrderRecord`). **무시된
스위치**는 바인딩에서 빠져도 조건으로 센다. 사용자 눈에는 조건이 걸린 액션이고, 무시가 캐릭터와
전문화마다 갈리므로 세지 않으면 공유 레이어의 순서가 캐릭터마다 갈린다. Cast Options의 값은 조건이
아니라 안 센다. Normal Cast의 Skip도 그렇다(`putting-conditions-back-in-the-order.md`).

**이 문서에서 "원본 순서"는 화면의 그 순서다.** 화면이 보여주는
