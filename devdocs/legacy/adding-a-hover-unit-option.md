...
- **매크로 변환.** `ConditionsSurviveMacroText`가 옵션이 켜진 액션을 `known`과 같이 거절한다. 옵션은
  ...
  거절이 맞다.
- **메뉴.** `DropDownMenus.lua`. ~~`IGNORE_HOVER_UNIT` 체크박스 옆~~ **대상 메뉴의 `ONLY_IF`
  아래, 구분선 하나 두고 (2026-09-06, 소유자).** hover 메뉴가 드는 것은 **언제 발동하느냐**이고 이
  상자는 언제를 안 건드린다 - 조건이 아닌 것을 조건 묶음에 두면 안 된다(§2). 짝이 된다는 것은 자리를
  같이 쓸 이유가 못 된다. 따라서 문제 갈래도 `hover`가 아니라 `unit`이다.
  활성 조건은 넷: 타입이 위 여섯 중 하나, hover 조건 꺼짐, Clique 꺼짐, **대상이 `none`도 `hover`도
  아님**. 뒤엣것도 소유자 것이다 - `hover`는 이미 그 개체를 겨누고, `none`은 대상 입력을 받는
  시전이라 겨눔을 가로챌 자리가 아니다. 안 맞으면 `SetEnabled(false)`, 값은 지우지 않는다.
  **잠그는 넷과 파생이 거절하는 넷은 같아야 한다**(`GetBindingsForAction`) - 공유 프로필은 이 메뉴를
  안 지나므로, 갈리면 잠긴 상자가 동작하거나 켠 상자가 아무 일도 안 한다.
- **로케일.** `enUS.lua`, `koKR.lua`. 말은 `writing-user-facing-text.md`대로 클라이언트 것 우선.
  개체창은 "개체창", 유닛은 "개체". 툴팁 한 줄에 "개체창 위에서는 그 개체창의 개체에게, 밖에서는 원래
  대상에게"가 들어가면 된다.
  **라벨에 조건절이 있어야 한다 (2026-09-06, 소유자).** "마우스 올린 개체에게 사용"처럼 언제인지를
  안 적으면 **언제나 개체창을 겨눈다**로 읽히고, 그건 hover 조건이 하는 일이라 이 상자가 있는 이유와
  정반대다. "우선"이 그 낱말이다. 툴팁에서 "평소처럼"도 뺀다 - 두 줄 위에서 방금 대상을 고른 사람에게
  거짓이다.
