...
> 상태: 전부 구현됨 (2026-09-05), **그 뒤 뒤집혔다 (2026-09-07)**. 이 문서가 닫은 문 셋과 홀더
> 앞에서 물러서는 규칙은 `legacy/standing-on-top-of-foreign-wrappers.md`가 되살렸다. 물러선 근거
> 둘 중 하나("남의 래퍼 위에서는 우리 leave가 죽는다")가 틀린 것으로 밝혀졌기 때문이다. 여기 남은
> 것 중 살아 있는 것은 `mouseover` 폴백, 전투 중 큐, 블리자드 체크박스 규칙, 홀더의 프록시 뒤에
> 서는 것이다. 근거는 `0-DIARY.md` 2026-09-05에 있고 여기엔 결론과 순서, 그리고 각 단계에서
> 계획과 갈린 것만 둔다.
