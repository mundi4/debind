...
> 상태 (2026-08-18): **끝났다. 둘 다 했다.**
>
> **1번** — `Drawer.lua`가 사라졌고 `Import.lua`가 한 덩어리다. 줄 두 함수는
> `Debind/ImportUI.lua`로 갔다.
>
> **2번** — 서랍이 페이로드를 저장한다. `STORE_VERSION`이 2가 되고 `MigrateToPayloads`가 옛
> 배치를 올린다. `class`/`groupCount`/`actionCount`가 레코드에서 빠지고 개수는 `CountBatch`가
> 물을 때마다 센다. `npm run check` 전부 통과하고, `/debtest`에
> `Import: a drawer written by store v1 comes forward`를 등록했다(아직 게임에서 안 돌렸다).
>
> **이 문서가 남기는 것은 결론이 아니라 잣대다.** 아래 2번이 문자열을 저장할 이유 넷을 하나씩
> 죽이는데, 죽인 방법이 **"A는 되는데 B는 안 되는 경우를 대라"**였고 그 한 줄이 넷을 한 번에
> 정리했다(`0-DECISION-LOG.md`, 소유자). 결정 자체는
> `building-export-import.md`의 "★ 서랍은 페이로드를 저장한다"가 든다.
...
바꾸기 전에는 **원문 문자열**을 저장했다. `batch.text`. 붙여넣을 때 한 번 역직렬화해서 읽을 수
없는 것을 거절하고 **그 결과를 버렸다.** 목록을 그리려고 그 결과에서 뽑은 `class`, `groupCount`,
`actionCount`만 배치 레코드에 따로 적어뒀고, `decoded`는 파일 지역 테이블이라 메모리 캐시였다.

즉 디스크에 원본과 그 파생값이 둘 다 있었다.
...
바꾸면 딸려 오는 것 — 넷 다 그대로 나왔다:

- `class`, `groupCount`, `actionCount`가 필요 없어진다. 페이로드에서 읽는다 → `CountBatch`
- 목록을 그리는 데 압축 라이브러리가 필요 없어진다(전에는 `Bring`이 다시 디코드했다)
- `STORE_VERSION` 밑에 마이그레이션 단계가 생긴다 → `STORE_VERSION = 2`, `MigrateToPayloads`
- 이미 저장된 배치(문자열)를 한 번 읽어 옮기는 단계가 필요하다

**계획에 없던 것 하나.** 마이그레이션이 못 읽는 배치를 어떻게 하느냐가 남아 있었다. **문자열을
든 채 서랍에 그대로 두고**, 열 때 이유를 낸다 — 지우면 로그인했더니 줄이 하나 없어져 있는 것이
되고, 그건 이 서랍이 내면 안 되는 유일한 결과다. 근거는 `building-export-import.md`의 ★ 절에.
...
- **임포트 목록은 디코드하지 않는다.** 행이 그리는 것은 `BatchTitle`(= `batch.source`),
  ...
  (2번을 하고 나서도 목록은 디코드하지 않는다. 앞의 둘이 `CountBatch`와 `batch.payload.class`로
  바뀌었을 뿐이고, 셈은 이미 서랍에 있는 테이블을 훑는 것이다.)
...
- ~~`tests/drawer_spec.lua`(이름이 바뀌든 말든)가 그대로 통과해야 한다. 옮기기만 하는 변경이다~~
  → `tests/batch_spec.lua`다. 1번에서 그대로 통과했고, 2번에서 저장 모양을 묻는 케이스 넷이
  새로 섰다.
- **2번을 하면 저장 데이터가 바뀌므로 `/debtest`가 필요하다.** 그리고 옛 배치가 든 프로필로
  한 번은 로그인해봐야 한다

  → `Import: a drawer written by store v1 comes forward`로 등록했다. **헤드리스가 원리적으로 못
  보는 자리가 하나 있어서다**: 스펙은 디코더를 스텁으로 갈아끼우는데(fengari가 압축을 못 푼다)
  스텁은 호출 모양이 뭐든 답하므로, 마이그레이션이 **진짜 문자열**을 읽느냐는 게임에서만 물을 수
  있다. **아직 게임에서 안 돌렸다.**
