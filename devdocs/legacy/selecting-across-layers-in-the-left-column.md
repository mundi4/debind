...
않는다. 사이드탭(`DebindSideTabMixin:OnClick`)도 같은 줄을 뺐다. 창의 탭(`SelectPanel`)과 Settings의 Back
(`LeaveSettings`)에는 원래 푸는 줄이 없다.
...
- **킷(`DebindDev/DebindTest.lua`, "Left column:"으로 시작하는 여섯).**
  - 두 레이어에서 고른 것이 둘 다 골라지고, 메뉴로 둘 다 넘어가고, 사이드탭, 레이어 탭, Switches에 갔다 오기, Settings와
    Back을 지나도 남는다.
  - 앵커가 안 그려져 있을 때 선택 전체인 그룹의 머리글에 SHIFT를 해도 그 그룹이 남는다.
