...
> 상태 (2026-08-21): **전부 들어갔다. 이 문서에 남은 일이 없다.**
>
> **08-20에 이 줄을 먼저 썼던 것은 틀렸다.** §4-1이 조건을 `conditions` 안으로 내리기만 하고
> 이름은 `checkedUnits` 그대로 뒀는데, 아래 §4 블록도 §7도 도착지를 `units`로 적어두고 있었다.
> 08-21에 `dbver 6`과 v1 -> v2 사다리 안에서 같이 갔다.
>
...
### 1. `conditions` 중첩 — **들어갔다 (2026-08-20). `dbver 6`. 이름은 08-21.**

**`checkedUnits`는 `units`가 됐다 (2026-08-21).** `conditions.` 접두어가 이미 "이건 조건이다"를
말하므로 `checked`가 같은 말을 한 번 더 했다. 옮기는 단계에 얹혀서 왔다 - `dbver <= 5`와
`Export.lua`의 v1 -> v2가 어차피 그 표를 지나가므로 따로 세운 단계가 없다. 옛 이름을 읽는
자리는 셋으로 닫힌다: 그 두 단계와, 마이그레이션 전 프로필을 보는 `GetBindingInfoForAction`의
`rawget`.
...
그대로 쓴다. `binding.priority`도 같이 나갔다. 되돌아가도 순서는 맞아서 화면에는 아무것도 안
나오므로, `/debtest`의 `Binding carries no ordering fields`가 그것을 잡는다.
...
원래 적어둔 것: 캐시에서 꺼내 제자리에서 덮어쓰는데, 조건부로만 쓰는 필드는 안 지워진다. `binding.spellName`이 그것이다(`type == SPELL`일 때만 대입, `else` 없음).
...
원래 적어둔 것: `action._dirty`를 **열한 곳이 쓰고 영 곳이 읽는다**
...
## 8. 언제 무엇을 내나 (2026-08-19에 정해짐, **2026-08-20에 소유자가 뒤집음**)

**셋 다 재설계를 안 기다리고 나갔다.** 아래 표는 정해질 당시의 것이고, 둘째 줄은 서지 않는다.
...
**깨진 자리는 전제였다.** 재설계가 `$state1`~`$state5`를 개명한다고 보고 "조건 키가 두 번
움직인다"를 배정 근거로 삼았는데, 재설계 문서 본문은 그 다섯을 유효한 이름으로 남기고 임의
이름을 **추가로** 허용한다고 이미 적고 있었다. 조건 키는 한 번만 움직인다
(`0-DECISION-LOG.md` 2026-08-20).
...
그리고 `CleanUpDB`의 `$` 탈출구가 둘의 공유 부품이다. `legacy/redesigning-custom-states.md`의 ⚑1에
