> 상태: §8의 세 단계를 구현했다. **§6-2(스위치 게이트)는 대상이 없어졌다.** 계산식 스위치를 매
> 틱 파싱하던 상태 루프와 `CollectSwitchGate`가 `3734155`에서 빠졌다. 2026-09-25에 legacy로 옮겼다. 어느 층이 무엇을 덮고 무엇을 원리상 못 보는지는 §7이 든다.
...
**지금 답이 무엇인가.** 리빌드 때 비보안 쪽에서 `SecureCmdOptionParse("[known:<값>]")`를 그대로
...
**두 갈래가 같은 주문을 넣으면 높은 레벨이 남는다.** 거짓을 굳히던 때는 낮은 쪽을 남기면 리빌드
없이 참이 될 답을 거짓으로 접게 됐다. 참만 굳히는 지금(§5)은 낮은 쪽도 안전하고, 높은 쪽은 둘 중
엄한 쪽일 뿐이다.
...
`UpdateBindings.lua`의 파생 축. **굽는 인자는 조건이 든 값이다**: 보통은 주문 이름이고, 클라이언트가
이름을 못 준 줄은 id이며, 값 없는 세 타입에서만 액션이 푸는 주문이다
(`making-known-a-spell-name.md`).

그 값을 표에 묻는다. 표에 값이 있고 그 값이 `UnitLevel("player")` 이하이면 이 주문의
답은 고정이다. 둘로 갈린다.

- **고정이고, 지금 참이다.** `known` 필드를 아예 안 굽는다. 조건이 이미 만족이다.
- **그 밖의 전부.** 그 값 그대로 `[known:<값>]`을 굽는다.

**참만 굳힌다** (2026-09-23, 소유자). 레벨로 배운 주문은 잊는 길이 없고, 특성은 전투 중에 못
바꾸며, 특성이 대체한 주문도 base가 아는 것으로 답한다. 거짓에는 그런 바닥이 없다. 버프나 주문
시전이 주문을 잠시 가르치거나 덧씌우면 리빌드 때의 거짓이 전투 중에 참이 되는데, 굳혀 두면 그
전투가 끝날 때까지 틀린 채로 선다. 전에는 고정이고 거짓이면 레코드를 키 목록에서 뺐다.
...
고정이고 지금 거짓인 바인딩은 화면에 이유가 있어야 한다. 굳히지는 않지만 지금 안 나가는 것은
같다(`Spells.KnownMissing`). 이미 있는 것을 쓴다. `row.noSpell`(`Profile.lua`)이 그 자리이고,
문구는 `ORDER_FLAG_NO_SPELL`("No spell")이다.
...
§5가 고정이라고 판정하는 주문이면 그 토큰은 이 리빌드 동안 상수다. **지금 참일 때만이다.** §5가
거짓을 안 굳히므로, 고정이어도 지금 거짓인 토큰은 아래에서 말하는 상수가 아니다. 상수는 답을 못 움직이므로
**플래그를 안 내는 토큰**으로 처리하면 된다. 지금처럼 "못 알아봤다"로 처리하지 않는다. 그러면
나머지 토큰이 전부 알아볼 수 있는 것일 때 스위치가 게이트를 되찾는다.

- **이름 인자도 같이 받는다.** 이 문단은 원래 "숫자만 받는다"였고, 근거는 이름에서 ID로 내려가는
  단계가 확실하지 않다는 것이었다. 그 단계가 없어졌다. 조건이 이름을 저장하고 고정 판정이 이름
  색인을 직접 치므로(`making-known-a-spell-name.md`), `[known:주문이름]`도 `[known:123]`과 같은
  갈래로 들어온다. `SwitchGateFlag`는 앞의 `no`를 이미 떼고 나서 단어를 보므로
  (`UpdateBindings.lua`) `noknown:` 쪽도 같다.
...
하나 줄고, 지금 런타임과 결과가 같아야 한다. 참이면 항상 통과하기 때문이다. 다만 이건 말로 한
논증이므로 §7이 스펙으로 잡는다.
...
- 굽기 갈래. 고정이고 참이면 필드 없음, 고정이고 거짓이면 `[known:<id>]` 그대로, 고정이 아니면
  역시 그대로. 아직 그 레벨에 못 이른 주문도 같이 본다. `_measuredStates` 내용과 `SPELLS_CHANGED`
  등록까지 같이 본다.
- 정적으로 참인 `known`이 발동 순서를 안 바꾸는지. 제한 환경에 그 주문을 **안 알려준 채로**
  물어야 뜻이 산다. 축이 남아 있으면 거기서 지므로, 이기는 것이 곧 축이 없다는 말이다.
- 고정이고 거짓인 레코드가 **개수로** 남아 있는지, 그리고 리빌드 뒤에 그 주문을 알게 되면 이기는지.
- 고정이고 거짓인 행에 `noSpell`이 서고 참인 행에는 안 서는지.
