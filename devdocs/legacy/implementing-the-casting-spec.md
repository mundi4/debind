...
> 두었는데, **그 절이 보류가 되었다** (2026-09-19, 소유자). 그 다음에 걸어 둔 그쪽 §7도 2026-09-22
> 도움말 페이지 정리가 `targeting.md`로 대신했다.
>
...
- 이슈 검사에 `"casting"` 갈래. 넷을 다 끈 액션은 WARNING이고, 바인딩이 없어 키에 안 선다. (2026-09-17에
  이슈에서 빠져 실행되지 않는 이유가 됐다. `legacy/reorganizing-binding-issues.md` §3-3.) **키는
...
- `TwinUnitFor`: `aim = "skip"`이면 쌍둥이를 안 만든다. **원본에 [모드의 유닛 없음]을 얹는다.** 조건 표에
  ...
  때만 `CASTING_NONE_LEFT`(WARNING)였고, 2026-09-17부터는 이슈가 아니다(`legacy/reorganizing-binding-issues.md` §3-3).
