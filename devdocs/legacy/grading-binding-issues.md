...
- **문장도 같이 고쳤다.** *"이 애드온의 일부 기능이 동작하지 않습니다"*가 저렇게 뭉갠 것은 Clique만
  켜져 있으면 무조건 떠서였다. 조건이 좁아지니 무엇이 멈췄는지를 말할 수 있다 — *"여기 걸어둔
  개체창 지정이 동작하지 않습니다."* 세 로케일 다 바꿨다.
  - **한 줄을 안 넘긴다**(소유자, 2026-08-17): *"챗창에 표시될 메시지라서 여러 줄 차지하는 건
    부담스럽다."* 전리품과 퀘스트 글이 흐르는 같은 프레임이다. 애드온 이름은 `_MESSAGE_PREFIX`가
    이미 붙이므로 문장이 다시 말하지 않는다.
