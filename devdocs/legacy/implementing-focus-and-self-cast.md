...
> 상태: **다 들어갔다.** §3은 모든 액션의 쌍둥이와 키를 네 층으로 펼치기(§3-4), 조합키 칸, 누를 때
...
> 쌍둥이가 싣는 유닛, §3-6의 `none`, §3-12의 액션 칸 잠금은 들어갔다.
>
...
> 앞자리이고 더 안 움직인다.
>
> **§7의 초안은 그대로 나가지 않았다.** 그 자리는 2026-09-22 도움말 페이지 정리가 `targeting.md`로
> 다시 썼다. 초안이 담으라고 한 둘 중 `Resolved Unit`이 어느 유닛에 검사되는지는 그 페이지의 한
> 문장이 들고, 조합키가 가리킨 유닛을 이기는 자리는 두 순서가 다르다는 사실 한 문장으로만 서 있다.
> 그 페이지는 답만 든다는 규칙(2026-09-17, 소유자)을 머리에 달고 있다.
...
> **이 단락의 마지막 결정은 2026-09-23에 뒤집혔다.** 변환은 대상을 안 고른 액션의 본문에 아무것도
> 안 적는다. `@@`는 누름이 유닛을 안 넘기면 `"target"`으로 떨어지므로(`COMPOSE_MACROTEXT_SNIPPET`),
> 클라이언트 기본 타게팅을 따르던 키가 변환 한 번으로 현재 대상 고정이 되었다. 조합키 시전과 Hover
> Cast를 잃는 쪽이 작은 변화라는 것이 뒤집은 근거다. `@@` 자체는 그대로 있고, 사용자가 본문에 적으면
> 전과 같이 동작한다.
...
누름에서 그 유닛인 것, 스위치 식이 `@@` 그대로인 것(`eval_spec`). 변환이 대상을 안 고른 액션의 본문에
아무 대상도 안 적는 것(`convert_spec`, `castname_spec`. 2026-09-23 전에는 `[@@]`를 적었다).
...
**나간 본문은 `targeting.md`다.** 아래 초안은 2026-09-13에 쓴 것이고, 2026-09-22 도움말 페이지
정리가 그 페이지를 다시 쓰면서 대체됐다. 무엇을 담으라고 요구했는지의 기록으로 남긴다.
