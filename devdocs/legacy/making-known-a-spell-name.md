...
> 대체되는 주문을 고정 판정에서 가르는 근거)가 남았고, 둘 다 지금 코드가 안전한 쪽으로 떨어져서
> 열린 채로 legacy로 옮겼다(2026-09-25, 소유자).
>
...
**타입 제한은 그대로 남는다** (2026-09-12, 소유자). 물음 자체는 이름이 되면서 액션과 무관해졌지만,
...
**그 셋도 굽는 자리에서는 이름으로 푼다**(2026-09-12, 소유자). 리빌드가 전문화의 주문을 집은 뒤
그 id의 이름을 물어 `[known:<이름>]`을 굽는다. 나가는 문자열이 한 모양으로 통일되고, 같은 주문을
묻는 다른 액션과 상태 키를 나눠 쓴다. 이름이 안 나오면 id가 그대로 나간다.

같은 규칙이 `true`를 든 주문 액션에도 걸린다. 마이그레이션을 지난 프로필에는 없는 모양이지만,
갈래를 둘로 두면 같은 물음이 두 문자열로 나간다.
