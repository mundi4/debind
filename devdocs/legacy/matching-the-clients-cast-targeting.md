...
> 붙었다(§3-1). 한 번 빠졌던 유지·시전 주문도 2026-09-21에 들어갔다(§2-2).
>
...
**유지·시전 주문은 한때 뺐다.** 매크로는 한 번 돌고, 누를 때 시작해 뗄 때 놓는 게이트의 두 엣지를
그 한 번이 못 나른다고 봤다. 감싸면 대상을 지킨 대신 주문이 안 끝난다는 것이 근거였다. 스마트캐스트
갈래는 어느 host를 달고 있든 평범한 주문이라 그때도 감쌌다.

**그 이유는 2026-09-21에 게임에서 뒤집혔고, 같은 날 들어갔다.** 본문을 엣지마다 다르게 쓰면
나른다. 측정과 구현은 `setting-the-clients-cast-automatics-per-action.md`의 측정 절이 든다.
...
대상 없는 주문이 자기 버튼 그대로 나가는가, 유지·시전 주문이 엣지마다 제 본문으로 감싸이는가, 시전 프레임의 `unit`에 무엇이
