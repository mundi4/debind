...
> 상태: 전부 구현됨 (2026-09-07). 착수 전에 게임 PoC가 통과했고, 그 프로브 파일은 1단계에서
> `/debtest` 케이스로 옮기며 지웠다. 근거와 다툼은 `0-DIARY.md` 2026-09-07 "물러설 이유가
> 남았나"에 있고, 여기엔 결론과 순서, 그리고 각 단계에서 계획과 갈린 것만 둔다.
> `legacy/leaving-unregistered-frames-alone.md`(2026-09-05)가 닫은 문을 다시 여는 문서다.
>
> **2026-09-11에 대신 도는 부분이 없어졌다.** 아래 "재조립 규칙" 3번부터 5번까지는 더 이상
> 코드가 아니다. 남의 unwrap을 무엇으로 읽느냐는 그날 따로 정했다.
> 떼어낸 것은 **전부 도로 끼우고** 우리 것만 맨 위에 얹으므로, enter와 click은
> 클라이언트가 사슬을 그대로 돈다. 우리가 대신 도는 것은 `OnLeave` 하나뿐이고, 그것도 사슬에
> 도로 끼운 채 헤더와 본문의 사본을 들고 돈다. 특히 규칙 5번의 "하나라도 `false`를 돌리면 우리
> 몫을 마친 뒤 우리도 `false`를 돌린다"는 틀린 조항이었다. 원래 사슬에서 위쪽의 `false`는 우리
> 몫까지 같이 끊는다. 지금은 우리가 맨 위라 아래의 `false`가 우리에게 닿지 않는다.
...
- **필드를 읽어 세우는 것은 3단계에 이미 들어갔다.** 여기 남은 것은 체크박스와 세 문의 게이트,
  그리고 문자열이다.
- **툴팁은 체크박스가 직접 든다.** 계획은 "블리자드 개체창 항목 옆"이라 했고 그 항목의 툴팁은
  일곱 체크박스를 담은 부모 버튼이 들고 있는데, 이 옵션에는 담을 것이 하나뿐이라 부모 버튼을
  세우면 항목 하나짜리 하위 메뉴가 된다. 대신 `SetTooltip`을 체크박스 자체에 걸고 설명 뒤에
  `REQUIRES_RELOAD`를 붙였다. 부모 버튼에 달면 리로드가 필요 없는 클릭 엣지 항목까지 그 말을
  뒤집어쓴다.
- `/debtest`의 EUI 케이스 셋은 `applies`에 옵션을 더했다. 꺼져 있으면 세 문이 아무것도 안 하므로
  그 상태에서 도는 케이스는 사용자의 설정을 재는 것이 된다.
- 두 케이스 중 "꺼둔 프로필이 꺼짐으로 읽힌다" 쪽이 고치기 전에 빨갛게 나오는 것을 봤다. 켬 쪽은
  "언제나 참"에서도 초록이라 그 짝으로만 선다.
...
- **`Profile.lua`의 `unitFrameNoticeSeen`과 `migration_spec`의 두 케이스는 이미 없었다.** 9월 7일
  일기의 "새 설치에도 표시해"가 그 갈래를 먼저 뺐기 때문이다. 실제로 지운 것은 `DebindUI.lua`의
  믹스인, `DebindUI.xml`의 창, `Events.lua`의 로그인 분기, `.luacheckrc` 전역 둘, 문자열 둘,
  그리고 네 스펙이 세워두던 `ShowUnitFrameNotice` 대역이다. 필드는 그대로 뒀다.
- **`README.md`는 163행 하나만 다시 썼다.** 26행("Clique 자체는 꺼야 한다")과 121행(전투 안팎의
  custom target)은 지금도 참이다.
- **`CHANGELOG.md`는 다섯 문단 중 넷을 다시 썼다.** "Set Custom Target" 문단은 `mouseover` 폴백이
  남아 있어 그대로 참이다. 처음 둘은 한 쌍으로 묶어 다시 썼고, 옵션을 끄는 법이 거기 들어갔다.

## 커버리지

헤드리스가 드는 것(`tests/reassemble_spec.lua` 일곱, `frames_spec` 아홉, `unitwatch_spec` 둘,
`holder_spec` 아홉, `migration_spec` 둘): 재조립 규칙 전부(위 첫 것만 붙는 leave, 전부 붙는
enter와 그 순서, `false`가 아래를 끊는 것, post의 `message`, 남의 unwrap 뒤 복귀, 되받아치는
상대에서 물러남, 그쪽 본문이 그쪽 환경에서 도는 것), 문 셋의 옛 케이스, 홀더 규칙의 옵션 양쪽,
옵션 기본값의 양쪽.

심이 클라이언트를 닮게 만든 것도 그 커버리지의 일부다. `wow_frames.lua`가 래퍼 사슬을 상태로 들고
`restricted.lua`의 `Interp:runWrapped`가 `_wrapentered`까지 포함해 그 사슬을 돈다. 그래서 "남이
위에 감싸면 우리 leave가 안 온다"가 헤드리스에서 재현된다.

`/debtest`가 드는 것: 가짜 헤더가 감싼 버튼에서 그쪽 키가 걸리고 풀리는 것과 우리 hover가
채워지고 비워지는 것, post 본문의 `message`, 그쪽 unwrap 뒤 우리가 맨 위인 것, 되받아치는 상대에서
셋 안에 멈추고 그쪽 본문은 계속 도는 것. EUI 케이스 셋과 홀더 케이스 둘. 헤드리스가 못 보는 것은
제한 환경이 남의 헤더 핸들에 대고 `RunFor`를 받아주는가와, 그쪽이 자기 본문에서 건 키가 우리를
지나 바인딩 표까지 닿는가다.

앞의 넷은 `setup_onenter_wrap`을 `SecureHandlerExecute`로 돌린다. 래퍼에 올라가는 것과 같은
글자라 순회와 그쪽 본문과 그쪽 환경은 전부 진짜다. **래핑된 스크립트를 직접 부르는 길은 없다.**
`CallRestrictedClosure`가 `issecure()`가 거짓이면 거절하고(`RestrictedExecution.lua`), 그 값이
참이 되는 것은 클라이언트가 C에서 핸들러를 부를 때뿐이다. `IsWrapEligible`은 그 문이 아니고
`securecall`도 못 씻는다. 처음에는 그렇게 몰다가 케이스 넷이 **아무 오류 없이 아무것도 안 하는**
것으로 나왔다.

닿지 못하는 것:

- **전투 중.** 킷은 락다운을 흉내 낼 수 없고, 보호된 프레임을 전투 중에 감싸는 것은 클라이언트가
  거절하므로 몰 것 자체가 없다. PoC 3이 게임에서 손으로 확인한 자리다.
- **실제 EUI의 HoverCast를 세션 중에 껐다 켜는 전환.** 킷은 EUI 설정을 바꾸지 않는다. 가짜 헤더가
  같은 호출 순서(`WrapScript` 둘, `UnwrapScript` 둘)를 내니 그 길은 가짜 헤더로 본다.
- **커서가 실제로 프레임을 오가는 것, 그리고 그것이 곧 블리자드가 우리 래퍼를 부르는 것.** 킷은
  커서를 못 옮기고, 위에 적은 이유로 스크립트를 대신 부를 수도 없다. 그래서 "우리가 맨 위에 있고
  블리자드가 거기서부터 내려온다"는 헤드리스의 사슬 모형과 착수 전 PoC가 손으로 본 것이 전부다.
- **C 스택 오버플로 자체.** 헤드리스에서는 가드를 빼도 Lua 재귀로 끝나고 같은 답이 나온다.
  깊이 198은 PoC가 게임에서 잰 값이고, 킷이 재는 것은 셋 안에 멈춘다는 쪽이다.
