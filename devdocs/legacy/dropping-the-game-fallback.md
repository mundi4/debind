...
> 상태: 구현 끝. BLOCK과 상태 루프의 키 몫 걷어내기(§9-4의 답), 계산식 스위치를 누를 때 재기와 박자 좁히기, 스위치 알림
> 전역 옵션, `GetHoveredUnit`, 대전 키 양보(§5-1), 행동 단축키 액션과 행동 칸 `COMMAND`의 마이그레이션(§4), 타입 메뉴와
> 카탈로그에서 두 타입 빼기, README, 저장된 두 타입의 이슈 표시와 X 아이콘(§3)이 들어갔다. 걷어내기로 했다 (2026-09-14, 소유자). 막히는 곳은 없다는 데까지 쟀다 (2026-09-14, retail과
...
- 새로 만드는 길만 막는다. 타입 메뉴와 카탈로그에서 뺀다. 행이 그 이름을 그리니 로케일 문자열(`TYPE_COMMAND`,
  `TYPE_UNUSED`)과 행 표시(`ActionDisplay.lua`)는 남기고, 이슈 문구가 더해진다. 두 타입의 행 아이콘은 빨간 X
  (`ReadyCheck-NotReady`)다 (2026-09-17, 소유자). README는 두 타입이 더는 안 나간다는 것을 적는다.
...
- **옮길 곳이 있는 액션만 마이그레이션한다.** 행동 칸을 누르는 `COMMAND`는 §4의 액션으로 옮긴다.
  ...
  쓴다. 공유 레이어에 든 것도 같다. 소환수 바와 태세 바(§4-8)도 대응표에 들었고, 나머지는 대신할 수 없다
  (§4-7, `VEHICLEEXIT`은 `/leavevehicle` 매크로가 된다).
...
- 정한 것: 걷어낸다(§2), BLOCK과 대응표(§3), 행동 칸 `COMMAND`만 마이그레이션하고 나머지는 저장된 채 파생에서 BLOCK과
  ...
  이슈 문구와 행 아이콘(§3).
- 대전에서 키를 지키고 싶은 액션의 예외는 두지 않았다(`0-IDEAS.md`).
