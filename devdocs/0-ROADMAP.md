...
| **3.3에 같이** | 액션 마이그레이션을 한 벌로 | `unifying-action-migration.md` |
...
**액션 마이그레이션 한 벌은 3.3에 붙지만 전부가 3.3을 기다리지는 않는다 (2026-08-21).**
전송 포맷이 저장 모양을 그대로 닮게 하는 일인데, 그 문서 4절이 순서를 이렇게 갈라놨다.
`payload.v` 가르기와 v1 어댑터 자리 만들기는 **§9-1과 독립이라 지금 해도 되고**, setstate 예외를
걷어내는 것만 §9-1과 **같은 편집**이어야 한다. 먼저 하면 선 위의 이름을 비트팩으로 되돌렸다가
다시 이름으로 바꾸는 두 번 일이 된다.
