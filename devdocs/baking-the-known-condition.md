> 상태: 제안이다. 아직 아무것도 구현하지 않았다. 어느 층이 무엇을 덮고 무엇을 원리상 못 보는지는
> §7이 든다.
...
`spellID -> true` 하나짜리 표다. 뜻은 "이 주문의 `known` 답은 다음 리빌드 전에 안 바뀐다"이고,
세 갈래를 한 표에 합친다.
...
- `Enum.SpellBookSpellBank.Player`만 돈다. **펫 뱅크는 안 돈다.** 소환으로 전투 중에 주문책이
  바뀐다.
- 항목마다 `C_SpellBook.GetSpellBookItemInfo`. `itemType`이 `Enum.SpellBookItemType.FutureSpell`
  이면 뺀다. 아직 안 배운 주문이다.
- `C_SpellBook.GetSpellBookItemLevelLearned(slot, bank)`이 값을 주고 그 값이
  `UnitLevel("player")` 이하이면 넣는다. 카탈로그가 이미 이 API로 "레벨 40에 배웁니다" 부제를
  그리고 있다(`ActionCatalog.lua:484`).
...
3-1만 레벨을 탄다. 만든 시점의 `UnitLevel("player")`을 캐시에 같이 적어두고 다르면 다시 만든다.
안 다시 만들어도 틀리지는 않는다. 새로 배운 주문이 표에 없어서 동적으로 떨어질 뿐이다(§2의 안전
방향).
...
주문 ID를 구한 다음 표에 물어서 셋으로 갈린다.

- **표에 있고, 지금 참이다.** `known` 필드를 아예 안 굽는다. 조건이 이미 만족이다.
- **표에 있고, 지금 거짓이다.** 이 리빌드 동안 이 레코드는 절대 안 맞으므로 **키 목록에서 뺀다.**
  런타임이 지금 하는 일(`SecureBindings.lua:1522`가 레코드를 건너뛰고 다음 레코드로 넘어간다)과
  결과가 같다.
- **표에 없다.** 지금 그대로 `[known:<spell>]`을 굽는다.
...
표에 잡히는 주문이면 그 토큰은 이 리빌드 동안 상수다. 상수는 답을 못 움직이므로 **플래그를 안
내는 토큰**으로 처리하면 된다. 지금처럼 "못 알아봤다"로 처리하지 않는다. 그러면 나머지 토큰이
전부 알아볼 수 있는 것일 때 스위치가 게이트를 되찾는다.
...
- **"플래그가 없다"와 "못 알아봤다"를 갈라야 한다.** `CollectSwitchGate`는 지금 아무것도 안
  ...
  안 재는 것이라 뜻이 맞다.
- 표에 없는 `known`은 지금 그대로 게이트를 포기한다.
...
- `FutureSpell` 제외, 레벨이 캐릭터 레벨보다 높은 항목 제외, 펫 뱅크 제외.
- PvP 슬롯을 nil에서 끊는지, `availableTalentIDs` 전부를 넣는지.
- 굽기 세 갈래. 표에 있고 참이면 필드 없음, 표에 있고 거짓이면 바인딩 없음, 표에 없으면
  `[known:<id>]` 그대로. `_measuredStates` 내용과 `SPELLS_CHANGED` 등록까지 같이 본다.
- 정적으로 참인 `known`이 발동 순서를 안 바꾸는지.
