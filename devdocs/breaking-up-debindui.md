...
> 갈 것인가만 다르다.
>
> **`fixing-what-the-review-found.md`를 먼저 읽어라.** 아홉 개가 열려 있고 그중 여럿이 여기서
> 옮길 파일에 산다. 코드를 옮긴 다음에 그 수정을 얹으면 그 문서가 가리키는 자리가 이미 없다.
>
> **툴팁 작업(`sharing-one-action-tooltip.md`)도 먼저다.** 그쪽이 `DebindUI.lua`의 426줄을
> 들어내고 호출부 둘을 고친다. 여기 손을 먼저 대면 정면으로 부딪힌다.
