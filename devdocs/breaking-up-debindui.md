...
가르는 것만으로는 절반이다. **스코프가 작아져도 luacheck는 여전히 아무것도 안 잡는다** - 위
"왜 안 잡히나"의 두 번째 문단이 이유다. 테이블 필드와 XML 리전은 원래 못 보고, 지역 변수는
`.luacheckrc`가 꺼놨다. 이번에 나온 넷 중 셋이 앞쪽이다.

그러니 여기서 정할 것이 하나 늘었다. **211/231/311을 다시 켤 것인가.** 켜면 지금 **34개**가
뜬다(211이 31, 231이 1, 311이 2). 분포는 `Debind.lua` 11, `DropDownMenus.lua` 6,
`UpdateBindings.lua`와 `Misc.lua` 각 4, `Events.lua` 3, `UnitWatch.lua`와 `DebindUI.lua` 각 2,
`Public.lua`와 `BindingContexts.lua` 각 1. **새 파일 셋은 0이다.** 껐을 때의 값이 어디에도 안
적혀 있어서, 켜는 것이 되돌리는 것인지 뒤집는 것인지가 아직 안 갈린다.
