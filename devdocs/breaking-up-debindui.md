...
3. **XML `parentKey` 검사를 만들 것인가.** 오탐 수를 세어보고 정한다.
   **반대 방향이 더 급하다는 것이 2026-08-22에 나왔다.** 원래 세어둔 것은 "선언됐는데 아무도 안
   읽는 리전"이고, 실제로 문 것은 **"읽는데 선언이 없는 리전"** 이었다 - 보관함 행이
   `self.SelectedHighlight`를 켜는데 그 텍스처가 XML에 없었다. 앞엣것은 죽은 코드가 쌓이는 것이고
   뒤엣것은 **게임에서만 나는 에러**다.
   
   오탐 걱정은 뒤엣것에도 그대로 있다. 상속받은 블리자드 템플릿의 리전(`self.Input`,
   `self.Text`, `DebindDialogTemplate`의 `ContentArea`)은 우리 XML에 선언이 없는 것이 정상이라,
   화이트리스트 없이는 그대로 빨개진다. 세어보고 정할 것이 그것이다.
