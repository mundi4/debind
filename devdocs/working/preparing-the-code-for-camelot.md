...
**파일을 가르는 것은 한 클라이언트에서만 도는 구현이 파일 하나를 채울 때다.** 그때는 패키저 태그가
아니라 TOC 파일 줄의 게임 타입 조건으로 가른다(6절이 적은 두 조건). 클라이언트가 읽으므로 개발 중에도
같게 돈다(6절이 첫 사례다). 코드 파일을 그렇게 가를 때는 그 전에 `tests/run.lua`의 목록이 갈래를
알아야 한다(5절). 지금 그런 코드 파일은 없다.
...
**정해졌다: 그룹은 레이어에 안 들어온다** (2026-09-25, 소유자). 전문화가 하나뿐인 직업은 전문화 레이어가
없다(`shipping-on-the-camelot-client.md` 5절). 남은 물음은 특성 조건이다. 특성 조건은 전문화 번호를 키로
저장하므로(`talents[specID]`) 두 그룹이 같은 조건을 쓰고, 판정은 그때 활성인 그룹의 특성으로 한다.
...
**들어갔다: 등급 고정** (2026-09-24, 소유자). 최고 등급만으로는 모자란다. 클래식에서는 마나를
아끼려고 낮은 등급을 일부러 쓰므로, 액션마다 "최고 등급"과 "이 등급 고정" 둘 다 된다.

- **저장된 주문 번호만으로는 둘을 못 가른다.** 2등급이 최고일 때 고른 액션은 2등급 번호를 들고,
  3등급을 배운 뒤에는 그것이 최고 등급이었는지 고정이었는지 남지 않는다. 그래서 액션의 `pinRank`가
  고정을 든다.
- **기본은 최고 등급**이다. 시전 이름에 등급을 안 붙인다.
- **고정한 액션은 저장된 번호의 부제를 붙여 굽는다**(`Healing Touch(Rank 1)`). 같은 주문의 최고 등급
  버튼과 섞이지 않게 버튼 캐시 키도 따로다. 매크로로 바꿀 때도 같은 이름을 쓴다.
- **고르는 자리는 액션 메뉴의 "주문 등급"**이다. 시전 메뉴 옆에, 등급이 있는 클라이언트의 주문 액션
  하나를 골랐을 때만 선다. "최고 등급"과 주문책의 같은 이름 항목들(`Client.SpellRanks`)을 줄로 둔다.
  `ShowAllSpellRanks`로 주문 목록에 낮은 등급 줄이 보일 때 그 줄을 고르면 처음부터 고정해서 들어간다.
- **액션에 다른 주문을 넣으면 고정은 풀린다**(`SetActionEntry`).
- **저장 형식에 필드가 늘었다.** `KEYS_TO_SAVE`, `Export.lua`의 필드 목록, `import_spec`의 실제 값 표.
  주문이 아닌 액션의 `pinRank`는 `CleanUpDB`가 걷어낸다. 등급이 없는 클라이언트는 이 필드를 읽기만
  하고 무시한다. 부제를 늘 붙이기 때문이다.
...
구현마다 붙는 케이스로 돈다. 층의 모든 함수는 두 세계에서 케이스를 가진다. 예외로
`actionmenutree_spec.lua`는 통째로 두 세계에서 돈다. 메뉴 모양이 카멜롯에서 달라지고(전문화가 하나뿐인
직업은 하위 메뉴가 없다), 그 스펙의 규칙은 어느 세계에서나 서야 하기 때문이다.
...
**들어갔다: 카멜롯에서 달라지는 문장은 `Locales/Camelot/`의 파일이 덮어쓴다** (2026-09-25, 소유자).
`Debind.toc`가 그 파일을 `locales.xml` 바로 뒤에 `[AllowLoadGameType camelot] [ExcludeLoadGameType standard]`를
달아 싣는다. **둘째 조건이 정식 서비스를 막는다.** 조건에 적힌 게임 타입을 클라이언트가 하나도 모르면 조건은
참이 되는데, 정식 서비스 TOC에는 `camelot`이 없어서 첫 조건만으로는 정식 서비스에서도 파일이 실렸다
(2026-09-25, 소유자가 화면에서 봄). 정식 서비스는 `standard`이고 카멜롯은 아니며(블리자드의
`standard, camelot` 줄), 두 클라이언트 모두 그 낱말을 안다. 애드온
TOC의 파일 줄 조건은 11.1.5부터 되고, 블리자드 TOC도 같은 줄로 파일을 가른다. 그래서 정식 서비스에서는
파일이 아예 안 실리고, 개발 중인 작업 트리에서도 배포판에서도 같게 돈다. 키는 enUS의 키를 그대로 쓰고,
`check:locales`가 그 파일의 키가 enUS에 있는지와 서식이 같은지를 본다.

첫 줄은 조건 메뉴의 이름과 툴팁 제목이다. `CONDITION_SPEC`과 `CONDITION_SPECS`를 "Classes"로 덮는다.
복수인 것은 이웃한 줄(`Talents`, `Units`)과 같이 체크할 목록을 여는 줄이라서이고, 같은 날 정식 서비스의
메뉴 이름도 "Classes/Specializations"로 바꿨다(소유자). 클라이언트 전역 `CLASS`는 단수라 쓰지 않았다.
**enUS만 둔다**(2026-09-25, 소유자). 이 파일은 로케일 가드가 없어서 다른 로케일의 번역도 영어로 덮는다.

고르지 않은 것과 그 이유.

- **부르는 자리에서 능력을 물어 키를 고르기.** 이 문서가 처음 적은 안이다. 키 하나를 부르는 자리가
  여럿이라(`CONDITION_SPEC`만 넷) 자리마다 같은 물음이 서야 하고, 새 자리가 물음을 빠뜨리면 아무 검사도
  못 본다.
- **로케일 파일 첫 줄에서 능력을 물어 `return`하기.** 파일은 카멜롯의 문장을 드는데 문턱은 전문화 수 같은
  능력 하나가 된다. 갈래 상수를 이름만 바꿔 둔 것이다(`shipping-on-the-camelot-client.md` 2절).
- **패키저 태그로 TOC 줄 가르기.** 태그는 패키저만 읽어서, 작업 트리를 링크한 개발 클라이언트에서는
  양쪽 다 그 파일을 싣는다.

카멜롯에 없는 기능의 문자열은 화면에 안 뜨고, 카멜롯에서 헛돌 뿐 거짓은 아닌 문장("전문화를 바꿀
때")은 그대로 둔다. 거짓이 되는 후보로 남은 것은 넷이다.
