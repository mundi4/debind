...
첫 줄은 조건 메뉴의 이름이다. `CONDITION_SPEC`을 클라이언트 전역 `CLASS`로 덮는다. 카멜롯의 직업 메뉴도
같은 낱말을 쓰고 이미 로케일마다 번역돼 있어서, 이 한 줄은 `Camelot/enUS.lua`만으로 세 로케일이 다
된다. 우리가 쓰는 문장을 덮을 때는 `Camelot/koKR.lua`처럼 로케일마다 파일을 두고 enUS 다음에 싣는다.
