...
**다음 일: 등급 고정** (2026-09-24, 소유자). 최고 등급만으로는 모자란다. 클래식에서는 마나를 아끼려고
낮은 등급을 일부러 쓰므로, 액션마다 "최고 등급"과 "이 등급 고정" 둘 다 되어야 한다.

- **저장된 주문 번호만으로는 둘을 못 가른다.** 2등급이 최고일 때 고른 액션은 2등급 번호를 들고,
  3등급을 배운 뒤에는 그것이 최고 등급이었는지 고정이었는지 남지 않는다. 그래서 액션에 고정 여부를
  따로 둔다.
- **기본은 최고 등급**이다. 지금 들어간 동작 그대로 시전 이름에 등급을 안 붙인다.
- **고정한 액션은 부제를 붙여 굽는다**(`Healing Touch(Rank 1)`). 이 등급 전에 `ComposeSpellCastName`이
  언제나 내던 모양이다.
- **고르는 자리는 액션 메뉴의 한 줄**이다. "최고 등급", 그리고 배운 등급들을 나열한다.
  `ShowAllSpellRanks`로 주문 목록에 낮은 등급 줄이 보일 때 그 줄을 고르면 처음부터 그 등급으로
  고정해서 넣는다.
- **저장 형식에 필드가 는다.** `Profile.lua`의 `KEYS_TO_SAVE`, `check-export-fields`가 맞춰 보는
  `Export.lua`의 필드 목록, 공유 문자열을 가져올 때의 검사가 같이 움직인다. 등급이 없는 클라이언트에
  건너간 고정 액션은 그 필드를 무시한다.

7절에서는 3단계 뒤, 쪼개기 앞에 둔다.
