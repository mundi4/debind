...
구현은 래퍼 서두에서 `checkselfcast`와 `checkfocuscast`를 클릭캐스팅 갈래에만 거짓으로 박는 것이다.
로그인 때 한 번이 아니라 매 클릭인 이유는 키 경로가 반대 답이고 둘이 같은 프레임에 도착하기
때문이다. `checkmouseovercast`는 이미 모든 경로에서 꺼져 있어서(§2) 줄이 필요 없다.
