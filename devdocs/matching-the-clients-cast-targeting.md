...
> 상태: **잰 것은 다 나왔고 코드는 없다.** 고칠 것 둘, 그대로 둘 것 둘, 그리고 그 넷을 한 줄로
> 묶는 규칙(§3)이 아래에 있다.
>
> **이 트랙은 지금 나가고 있는 것을 고친다.** 새 기능은 `adding-hover-and-mouseover-cast.md`가
> 들고, 그쪽을 접더라도 여기는 답이 필요하다.

와우에는 액션이 어느 유닛에게 가는지를 트는 장치가 넷 있다. 우리 키도 그 장치를 지나가는데,
둘은 제자리에서 돌고 둘은 어긋나 있다.

| | 무엇 | 우리 키에서 |
|---|---|---|
| Mouseover Cast | 마우스 밑 유닛으로 튼다 | **틀린 판정을 받는다** (§2) |
| Self Cast | 수식 키를 누르면 자신에게 | 제자리 (§2-1) |
| Focus Cast | 수식 키를 누르면 포커스에게 | 제자리 (§2-1, §2-3) |
| 자동 자가시전 | 못 쓸 대상이면 자신에게 | **절반만 돈다** (§2-2) |
...
`adding-hover-and-mouseover-cast.md`가 우리 쪽 Mouseover Cast를 만든다. 그 트랙을 하든 안 하든
이 줄은 꺼야 한다.

## 2-1. Self Cast와 Focus Cast는 그대로 둔다
...
지금 `checkmouseovercast`가 이 둘보다 **앞에** 서 있어서(§1의 블록 순서), 1번 칸 판정이 걸리면
수식 키를 누르고 있어도 마우스오버로 나간다. §2를 고치면 이 둘이 제자리를 찾는다.

**프레임 클릭 경로에서는 셋 다 안 돈다.** 개체창 클릭은 그 프레임의 유닛으로 `unit`이 차서
나가고, `unit`이 있으면 `SecureButton_GetModifiedUnit`이 첫 줄에서 반환한다. 끄려고 속성을 박아도
동작이 같고 매 클릭 확정해야 할 속성만 는다.
...
**끄는 길은 매크로 몸통이다** (2026-09-11, 소유자).

    /run <이전 값 기억> SetCVar("autoSelfCast", 0)
    /click DebindClickButton debind-123
    /run SetCVar("autoSelfCast", <이전 값>)

시전은 그대로 그 버튼이 한다. `/click`이 닿는 곳에는 `*type-debind-123`과 `*spell-debind-123`이
이미 구워져 있어서 보안 경로가 안 바뀐다. 앞뒤 두 줄만 감싸는 것이다.

**우리가 속성으로 넣은 몸통 안에서 Lua가 돈다.** `DevSeed.lua`의 hover 행들이 이미
`/script print(...)`를 그렇게 굽고 있고 게임에서 돌고 있다. 이 길이 열려 있다는 증거가 리포 안에
있었다.

**주문과 아이템만이다.** 매크로 타입은 바깥이 매크로가 되면 안쪽 매크로가 안 돈다
(`FrameRegistry.lua`의 옛 경로 주석). 그리고 자동 자가시전이 걸리는 것도 그 둘뿐이다.
...
**헤드리스가 덮는 것.** 우리가 내놓는 값까지다. `checkmouseovercast`가 꺼져 있는가, 매크로 몸통이
어떤 문자열로 구워졌는가, `unit`에 무엇이 들어갔는가. 전부 값을 견주는 일이라 스펙으로 잠근다.
...
- **`IsSpellHelpful`이 주문 대체를 풀고 답하는가.** 푼다면 변신이나 버프로 성향이 움직인다.
  클릭 때 묻기로 한 이유가 이것이라 답이 어느 쪽이든 설계는 안 바뀐다
  (`adding-hover-and-mouseover-cast.md` §1). 마법사 191034으로 찍어 보기로 했고 결과는 아직
  없다.
...
- **자동 자가시전이 아이템에도 걸리는가.** 클라이언트 문구는 주문만 말한다. 소유자는 걸릴
  것으로 본다.
- **`SetCVar`가 전투 중에 되는가.** 문서에 `RequiresNonSecureCVar` 같은 조건은 있는데 전투 잠금
  얘기는 없다 [CVarDocumentation.lua:118-135]. §2-2가 그 호출을 쓰므로 답이 필요하다.
