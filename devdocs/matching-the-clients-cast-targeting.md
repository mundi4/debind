...
> 상태: **셋 다 들어갔다.** `checkmouseovercast`를 껐고(§2), 대상을 지목한 주문·아이템이 자동
> 자가시전을 끄고 나가며(§2-2), 도움말 한 편이 붙었다(§3-1). 남은 것은 하나, **유지·시전 주문은
> 아직 옛 동작 그대로다** - 매크로 한 겹이 누름과 뗌을 못 나른다(§2-2).
>
...
| Self Cast | 수식 키를 누르면 자신에게 | 제자리 (§2-1) | 그대로 |
| Focus Cast | 수식 키를 누르면 포커스에게 | 제자리 (§2-1, §2-3) | 그대로 |
...
읽히고 켜 둔 사람이 키가 고장 났다고 본다. 대신 같은 일을 하는 `Unit Frame` 대상을 그 자리에서
가리킨다.
