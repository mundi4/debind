...
> 붙었다(§3-1). 남은 것은 하나, **유지·시전 주문은 아직 옛 동작 그대로다**(§2-2). 그 자리에
> 적혀 있던 이유("매크로 한 겹이 누름과 뗌을 못 나른다")는 2026-09-21에 게임에서 뒤집혔다.
> 빠져 있는 것은 그대로지만 못 넣을 이유는 없다.
>
...
**위 두 문단의 키 누름 쪽은 2026-09-13에 뒤집혔다가 2026-09-15에 돌아왔다.** 판단은 블리자드가 아니라
우리 래퍼가 하고, 대상을 고른 액션은 조합키를 안 따른다(`implementing-focus-and-self-cast.md` §3-1). 아래의
개체창 클릭 결정은 그대로다.
...
`useOnKeyDown`은 그 프레임에 거짓으로 박는다. 우리가 적는 `/click`이 엣지를 안 실어서 언제나
up으로 도착하는데, 사용자의 `ActionButtonUseKeyDown`이 켜져 있으면 게이트가 거짓이 되어 **아무
일도 안 나고 아무 말도 안 난다.** 클릭캐스팅 갈래가 같은 이유로 같은 값을 박는다.

**안 싣는 것이지 못 싣는 것이 아니다** (2026-09-21). `/click`은 세 번째 토큰을 받아
`StringToBoolean`을 거쳐 `Click(mouseButton, down)`에 넘긴다([SlashCommands.lua:728-739]).
`down`이라고 적으면 첫 글자가 어느 목록에도 없어서 기본값 거짓으로 떨어지고, 참으로 실으려면
`1`이나 `true`나 `on`이다. 이 절이 오래 "못 싣는다"로 적혀 있었고 그 문장 하나가 아래 유지·시전
주문 배제의 근거였다.
...
한 번이 못 나른다. 감싸면 대상을 지킨 대신 주문이 안 끝난다.

> **그 이유는 2026-09-21에 게임에서 뒤집혔다** (`DebindDev/Probe_PressHold.lua`). 본문을 엣지마다
> 다르게 쓰면 나른다. 누를 때 `/click ... true`, 뗄 때 `/click ... false`를 보낸 경로가 강화 시전
> 주문을 정상으로 차오르게 하고 뗀 뒤 0.21초에서 0.25초 사이에 끝냈다. 직접 경로와 같은 모양이고,
> CVar 감싸기를 더한 경로도 같았다. `ActionButtonUseKeyDown`을 끄고 다시 재도 같다.
>
> 같이 나온 것 둘. **본문 하나로 두 엣지를 보내면 시작만 걸리고 안 끊긴다.** 엣지 토큰이 없는
> `/click` 한 줄짜리 경로는 차오르기는 했는데 뗀 것이 안 닿아 2초 뒤에 혼자 끝났다. 그리고
> **마우스 클릭도 유지를 시작시킨다.** `isSecureMousePress`를 읽고 안 된다고 단정했었는데 화면의
> 버튼을 왼클릭한 것에서 그대로 차올랐다.
>
> 빠져 있는 것은 아직 그대로다. 넣는 것은 본문을 둘로 갈라 쓰는 일이고, 이 절의 지금 구현은
> 본문을 하나만 굽는다. 그래서 그 주문들은 지금 동작 그대로
