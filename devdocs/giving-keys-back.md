...
> `UpdateGivenBackKeys`와 그것을 깨우는 속성 드라이버까지. 남은 것은 8절 하나다.
>
...
| 무엇 | 어디 |
|---|---|
| 스킨 없는 바는 12개, 스킨 있는 바는 6개, 대전은 5개 | `giveback_spec` |
...
| 집 편집기 줄의 켬·끔, 한 명령의 키 둘이 다 넘어가는 것 | `context_spec` |
| 본문이 lua5.1로 컴파일되는지, 문구가 로케일에 다 있는지 | `npm run check` |

**원리상 안 재지는 것.** `ClearBinding` 뒤에 그 키를 눌렀을 때 게임 자신의 바인딩이 실제로
나가는가, 그리고 전이가 도착하는가. 뒤의 것은 블리자드 매니저가 드라이버를 푸는 일이고 헤드리스에는
매니저가 없다. 설정 줄 배선도 같다.
