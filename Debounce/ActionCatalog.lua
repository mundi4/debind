...
local function AddOwnCommands(Bucket)
	...
	local typeNames = DebounceUI.BINDING_TYPE_NAMES;
	...
	for _, actionType in ipairs(UNIT_ACTION_TYPES) do
		...
		for _, unit in ipairs(DebounceUI.SORTED_UNIT_LIST) do
...
local function BuildSpecialActions(entries)
	local seen = {};
	local typeNames = DebouncePrivate.DebounceUI.BINDING_TYPE_NAMES;
