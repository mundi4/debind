...
L["COMPARTMENT_TOOLTIP_LEFT_CLICK"] = "Left click to open Debounce."
...
L["ORDER_BLOCKED_PRIORITY_FIX"] = "Importance is compared before anything else. Click either row to set its importance directly. Changing a character-specific action only affects this character; changing a shared one affects every character, and this window cannot show you what happens on them."
L["ORDER_FLAG_CONDITIONAL"] = "conditional"
L["ORDER_FLAG_HOVER"] = "hover"
L["ORDER_FLAG_ISSUE"] = "has a problem"
L["ORDER_FLAG_UNREACHABLE"] = "never runs"
L["ORDER_LAYER_LABEL"] = "%s / %s"
L["ORDER_LINE_TOOLTIP_INSTRUCTION"] = "Click to set how important this action is."
...
L["PET"] = "Pet"
L["PRIORITY_DESC"] = "The same key can be assigned to more than one action. When you press it, Debounce tries them in order and runs the first one whose conditions are met -- only one of them ever runs.|n|nImportance is compared first, so it beats everything below it. Between actions that are equally important, the order is decided by:|n|n1. Hover -- an action that only runs while the mouse is over a unit frame is tried first.|n2. Conditions -- an action with conditions is tried before one without.|n3. Tab -- the more specific tab is tried first, from this character and specialization down to shared.|n4. List position -- higher in the list is tried first. Drag to change it."
...
L["TYPE_COMMAND_DESC"] = "Use these actions to run one of WoW's own binding commands -- jump, open a bag, press an action bar button, and the rest of its key binding list. Held in a Debounce action, a command can carry conditions, so the same key runs it only in the situations you pick."
