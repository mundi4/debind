...
--
-- 이 술어 하나가 "어느 키를 클릭 시점 평가로 보낼 것인가"를 정한다. 거짓 쪽으로 틀리면
-- 아무 일도 안 일어나지만(그 키가 옛 경로에 남을 뿐), **참 쪽으로 틀리면 키가 조용히
-- 죽는다** - 해제됐어야 할 키가 계속 걸려 있거나, SetBinding으로 나갔어야 할 명령이
-- 클릭 프레임으로 가서 아무 일도 안 한다. 그래서 거짓이어야 하는 경우를 더 촘촘히 본다.
--
